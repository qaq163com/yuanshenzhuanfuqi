...
def 检查国际服依赖包是否下载():
    依赖包是否下载 = True
    try:
        官转国大小 = 获取文件大小("data/官转国.zip")
        国转官大小 = 获取文件大小("data/国转官.zip")
    except:
        官转国大小 = 0
        国转官大小 = 0
    if 官转国大小 < 270:
...
if __name__ == '__main__':
    ...
    查找原神按钮 = tkinter.Button(tk, text='查找原神', font=字体大小, command=lambda: 创建新线程(查找原神))
    ...
    获取兑换码按钮 = tkinter.Button(tk, text="获取兑换码", font=字体大小, command=获取兑换码)
    """---------------------------------  布局设置  ---------------------------------------------"""
    ...
    查找原神按钮.place(x=第四个, y=第一行, width=200, height=50)
    """--------------------------------     第二行     -------------------------------------------"""
    ...
    if 获取兑换码(False):
        获取兑换码按钮.place(x=第二个, y=第三行, width=230, height=50)
    """---------------------------------  窗口启动  ---------------------------------------------"""
    tk.mainloop()
